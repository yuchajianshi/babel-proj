...
export default class Account extends React.Component {
  ...
  componentDidMount() {
    ...
    client.listen(res => {
      if (res.from == 'clip') {
        ...
        this.setState({
          babyInfos: this.state.babyInfos
        });
      }
    });
  }
  ...
  render() {
    return <View style={style.container} onTouchDown={() => {
      this.textInput && this.textInput.blur();
    }}>
        <Navigator title={'编辑资料'} />
        <View style={style.edit}>
          <ScrollView ref={r => this.scroll = r} horizontal={true} showsHorizontalScrollIndicator={false} onScrollEndDrag={e => this.onScrollEndDrag(e)}>
            {this.state.babyInfos.map((item, index) => <BabyCard first={index == 0 ? true : false} last={index == this.state.babyInfos.length - 1 ? true : false} ref={`child${index}`} name={item.name} birthtime={item.birthtime} avatar={item.avatar} kid={item.kid} key={item.kid} showLoading={show => this.showLoading(show)}></BabyCard>)}
          </ScrollView>
        </View>
        {this.state.babyInfos && this.state.babyInfos.length > 1 && this.state.babyInfos[0].kid && <Text style={style.button_delete__text} onClick={() => this.onClickDelete()}>
            删除宝宝信息
          </Text>}
        <Toast ref={toast => this.toast = toast} mode={'error'} />
        <Dialog ref={confirm => this.confirm = confirm} title={'提示'} config={{
        text: '确认删除宝宝吗？',
        btns: [{
          type: 2,
          text: '取消'
      ...
      }} onConfirm={() => this.onConfirm()}></Dialog>
